...
```json5
{
  session: {
    ...
    maintenance: {
      ...
      pruneAfter: "30d",
      maxEntries: 500,
      preserveRecent: "72h", // optional duration or false
      preserveActiveWorktrees: true, // optional, default false
...
- **`maintenance`**: session-store cleanup + retention controls.
  ...
  - `preserveRecent`: optional duration that protects recently active interactive sessions from automatic age, count, and disk-budget eviction (for example `"72h"`). Unset or `false` disables this protection. Synthetic model-run, cron, hook, heartbeat, ACP, and sub-agent rows remain eligible for bounded cleanup.
  - `preserveActiveWorktrees`: protect sessions while their managed worktree remains active. Default `false`; protection ends when the worktree registry marks the checkout removed.
