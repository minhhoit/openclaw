...
export const AUTOMATION_FIELD_HELP: Record<string, string> = {
  ...
  "session.maintenance.preserveRecent":
    "Protects interactive sessions active within this duration (for example `72h`) from automatic age, count, and disk-budget eviction. Unset or `false` keeps the normal oldest-first policy. Synthetic model-run, cron, hook, heartbeat, ACP, and sub-agent rows remain eligible for bounded cleanup.",
  "session.maintenance.preserveActiveWorktrees":
    "Protects sessions while their managed worktree remains active. Disabled by default. Protection ends when the worktree registry marks the checkout removed.",
